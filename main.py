...
# Get List of existing Articles from database
list_existing_article_ids=fetch_records_psql(database_url)

# Initialize List where we will put new Articles
driver=get_chromedriver()  # chromedriver will be used to scrape webpages
list_new_articles = ListNewArticles(driver, list_existing_article_ids)  # list_existing_article_ids will be used to check if article is new

type1="woning"  # We will do two types of searches (type1 and type2 will also be the titles of the emails sent)
type2="bouwgrond"

# Add new found articles to the list:
# Replace 'url' variables with your own urls. Make sure everything is sorted chronologically from newest to oldest!
#   IMMOSCOOP #
...
#   IMMOWEB #
url="https://www.immoweb.be/nl/zoeken/huis/te-koop/leuven/3000?countries=BE&orderBy=newest"  # again, make sure everything is sorted from newest to oldest --> notice the orderBy=newest
...
# Insert new found articles into database
insert_into_psql(list_new_articles.get_list_article_id(), database_url)

# Send Emails with new found articles
